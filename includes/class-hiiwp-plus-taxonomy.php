...
class HiiWP_Plus_Taxonomy {
	...
	public function __construct ( $post_type = 'portfolio', $token = 'work', $singular = 'Type', $plural = 'Types', $args = array() ) {
		...
		if ( '' == $this->singular ) $this->singular = __( 'Type', 'hiiwp-plus' );
		if ( '' == $this->plural ) $this->plural = __( 'Types', 'hiiwp-plus' );
	...
	private function _get_default_labels () {
		return array(
			    'name'                => sprintf( _x( '%s', 'taxonomy general name', 'hiiwp-plus' ), $this->plural ),
			    'singular_name'       => sprintf( _x( '%s', 'taxonomy singular name', 'hiiwp-plus' ), $this->singular ),
			    'search_items'        => sprintf( __( 'Search %s', 'hiiwp-plus' ), $this->plural ),
			    'all_items'           => sprintf( __( 'All %s', 'hiiwp-plus' ), $this->plural ),
			    'parent_item'         => sprintf( __( 'Parent %s', 'hiiwp-plus' ), $this->singular ),
			    'parent_item_colon'   => sprintf( __( 'Parent %s:', 'hiiwp-plus' ), $this->singular ),
			    'edit_item'           => sprintf( __( 'Edit %s', 'hiiwp-plus' ), $this->singular ),
			    'update_item'         => sprintf( __( 'Update %s', 'hiiwp-plus' ), $this->singular ),
			    'add_new_item'        => sprintf( __( 'Add New %s', 'hiiwp-plus' ), $this->singular ),
			    'new_item_name'       => sprintf( __( 'New %s Name', 'hiiwp-plus' ), $this->singular ),
			    'menu_name'           => sprintf( __( '%s', 'hiiwp-plus' ), $this->plural )
			  );
